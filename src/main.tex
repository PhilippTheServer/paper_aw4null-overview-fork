...
\begin{itemize}
  \item Clear error information delivery: easy to interpret, specific to identify
  \item User-friendly hardware design: minimal physical controls, intuitive handling
  \item Accessible software interface: quick installation, minimal user interaction
  \item Low entry barrier for training: easy to demonstrate, multilingual support
  \item Robustness against physical damage
  \item Cost-effectiveness in operation: setup effort must not exceed time/resource savings
  \item Prevention of misdiagnosis costs: avoids incorrect usage and unnecessary repairs
\end{itemize}
...
By automating signal interpretation, the implementation of such functions can reduce the need for manual waveform analysis, 
